...
class BpySceneCtx:
    ...
    def add_walls(self, walls: List[Dict[str, Any]]):
        ...
        # 过滤 height<=0 的墙
        valid_walls = [w for w in walls if w.get("height", 0) > 0]
        if len(valid_walls) < len(walls):
            print(f"🏞️ 室外场景：跳过 {len(walls) - len(valid_walls)} 面高度为0的墙")
        # walls_converted: [{"s": [x1, y1], "e": [x2, y2], "height": h}, ...] 统一后的墙体列表格式
        walls_converted = [{"s": w["p"][:2], "e": w["q"][:2], "height": w["height"]} for w in valid_walls]
